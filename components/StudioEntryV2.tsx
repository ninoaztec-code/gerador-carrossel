...
function unwrapRemote(raw: unknown, projectId: string): RemoteProject {
  ...
  return {
    ...
    template: String(nested.template || firstTemplate?.template || "T01").toUpperCase(),
    cards,
  } as RemoteProject;
}
...
function photoIdOf(card: RemoteCard) {
  if (card.photo_id) return String(card.photo_id).toUpperCase();
...
function editorStateIsUsable(project: RemoteProject) {
  ...
  const cardIndexes = new Set(project.cards.map((card, position) => Math.max(0, Number(card.card || position + 1) - 1)));
  const savedCopies = Object.keys(state.copies || {}).filter((key) => key.startsWith(`${state.templateId}:`)).length;
...
function buildState(project: RemoteProject): ProjectState {
  ...
  cards.forEach((card, position) => {
    ...
    const pKey = photoKey(template, index, slotIndex(card));
    copies[cKey] = {
...
export default function StudioEntryV2() {
  ...
  useEffect(() => {
      ...
      timer = window.setInterval(async () => {
        ...
        try {
          ...
          const payload: RemoteProject = {
            ...remote,
            template: editorState.templateId.toUpperCase(),
          ...
          // O autosave local continua sendo a fonte de recuperação no navegador.
        }
      }, 1400);
    };
    ...
    const boot = async () => {
      ...
      try {
        ...
        const remote = unwrapRemote(await response.json(), projectId);
