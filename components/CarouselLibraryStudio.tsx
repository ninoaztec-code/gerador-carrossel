...
import {INSTAGRAM_45PLUS_LIBRARY,LIBRARY_COLORS,LibraryCard,LibraryTemplate,Box,TextBox} from "@/lib/instagramTemplateLibrary";
...
function uploadFile(cb:(v:string)=>void){return(e:React.ChangeEvent<HTMLInputElement>)=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>cb(String(r.result));r.readAsDataURL(f)}}
...
function CardCanvas({template,card,index,images,photoCfgs,copy,brand,onSelectSlot}:{template:LibraryTemplate;card:LibraryCard;index:number;images:Record<string,string>;photoCfgs:Record<string,PhotoCfg>;copy:Copy;brand:Brand;onSelectSlot:(i:number)=>void}){
 const bg=colorOf(card.bg);const dark=[LIBRARY_COLORS.terracota,LIBRARY_COLORS.vinho,LIBRARY_COLORS.marrom,LIBRARY_COLORS.preto].includes(bg as any);const ink=dark?"#FFF9F4":"#493731";
...
export default function CarouselLibraryStudio(){
 ...
 const template=INSTAGRAM_45PLUS_LIBRARY.find(t=>t.id===templateId)||INSTAGRAM_45PLUS_LIBRARY[0],card=template.cards[cardIndex],pkey=slotKey(template.id,cardIndex,slotIndex),ckey=copyKey(template.id,cardIndex),pcfg=photoCfgs[pkey]||DEFAULT_PHOTO,copy=copies[ckey]||DEFAULT_COPY;
 ...
 const exportAll=async()=>{for(let i=0;i<5;i++){await exportOne(i);await new Promise(r=>setTimeout(r,200))}};
 ...
 return <main style={{minHeight:"100vh",background:"#111",color:"#fff",padding:22,fontFamily:"Arial,sans-serif"}}><div style={{maxWidth:1500,margin:"auto",display:"grid",gridTemplateColumns:"410px 1fr",gap:24}}>
  <aside style={{background:"#202020",borderRadius:18,padding:18,maxHeight:"calc(100vh - 44px)",overflowY:"auto"}}><h1 style={{margin:"0 0 4px"}}>Carousel Studio · Biblioteca 45+</h1><p style={{opacity:.65,marginTop:4}}>12 templates · 60 layouts-base · 1080×1350</p>
   ...
   <hr style={{borderColor:"#333",margin:"16px 0"}}/><strong>Card {cardIndex+1}/5</strong><div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:5,marginTop:8}}>{template.cards.map((_,i)=><button key={i} onClick={()=>{setCardIndex(i);setSlotIndex(0)}} style={{fontWeight:i===cardIndex?800:400}}>{i+1}</button>)}</div>
   <label style={{display:"block",marginTop:12}}>Área de foto</label><select value={slotIndex} onChange={e=>setSlotIndex(+e.target.value)} style={control}>{card.photos.map((_,i)=><option key={i} value={i}>Foto {i+1}{card.labels?.[i]?` · ${card.labels[i]}`:""}</option>)}</select>
